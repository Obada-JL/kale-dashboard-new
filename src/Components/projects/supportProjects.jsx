...
export default function SupportProjects() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/support-projects`
      );
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      const url =
        modalMode === "add"
          ? `${API_BASE_URL}/api/support-projects`
          : `${API_BASE_URL}/api/support-projects/${selectedProject._id}`;
  ...
  const handleDeleteProject = async (id) => {
    if (window.confirm("هل أنت متأكد من حذف هذا المشروع؟")) {
      setLoading(true);
      try {
        await axios.delete(`${API_BASE_URL}/api/support-projects/${id}`);
        fetchProjects();
      } catch (error) {
            ...
            <tbody>
              {projects.map((project) => (
                <tr key={project._id}>
                  <td>
                    <img
                      src={`${API_BASE_URL}/uploads/support-projects/${project.image}`}
                      alt={project.title}
                      style={{
                        width: "50px",
                        height: "50px",
          ...
          {viewProject && (
            <div className="view-project-details">
              <div className="text-center mb-4">
                <img
                  src={`${API_BASE_URL}/uploads/support-projects/${viewProject.image}`}
              ...
              {viewProject.details && (
                <div className="project-details mt-4">
                  ...
                  <div className="text-center my-3">
                    <img
                      src={`${API_BASE_URL}/uploads/support-projects/${viewProject.details.image}`}
                      alt="تفاصيل"
                      className="img-fluid"
