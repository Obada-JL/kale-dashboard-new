...
const DessertsPage = () => {
    ...
    const fetchCategories = async () => {
        try {
            const response = await apiService.categories.getDessertCategories();
            setCategories(response.data);
        } catch (error) {
            handleApiError(error, 'جلب فئات الحلويات');
        }
    };

    const fetchProducts = async () => {
        try {
            const response = await apiService.desserts.getAll();
            setProducts(response.data);
        } catch (error) {
            handleApiError(error, 'جلب الحلويات');
        }
    };

    const fetchImages = async () => {
        try {
            const response = await apiService.images.dessert.getAll();
            setImages(response.data);
        } catch (error) {
            handleApiError(error, 'جلب صور الحلويات');
        }
    };
    ...
    const handleAddProduct = async (e) => {
        ...
        try {
            ...
            await fetchProducts();
            toast.success('تم إضافة الحلوى بنجاح');
        } catch (error) {
            handleApiError(error, 'إضافة حلوى');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUpdateProduct = async (e) => {
        ...
        try {
            setIsLoading(true);
            await apiService.desserts.update(editingProduct._id, editingProduct);
            setEditingProduct(null);
            await fetchProducts();
            toast.success('تم تحديث الحلوى بنجاح');
        } catch (error) {
            handleApiError(error, 'تحديث حلوى');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleAddImage = async (e) => {
        ...
        try {
            ...
            await apiService.images.dessert.add(formData);
            setImageFile(null);
            setSelectedCategory('');
    ...
    const handleDeleteProduct = async (productId, productName) => {
        ...
        try {
            setIsLoading(true);
            await apiService.desserts.delete(productId);
            await fetchProducts();
            toast.success('تم حذف الحلوى بنجاح');
        } catch (error) {
            handleApiError(error, 'حذف حلوى');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeleteImage = async (imageId) => {
        ...
        try {
            setIsLoading(true);
            await apiService.images.dessert.delete(imageId);
            await fetchImages();
    ...
    const handleExportProducts = () => {
        try {
            const success = exportToExcel(filteredProducts, 'desserts', 'الحلويات');
            if (success) {
                toast.success('تم تصدير الحلويات بنجاح');
            } else {
                toast.error('فشل في تصدير الحلويات');
            }
        } catch (error) {
    ...
    const handleExportImages = () => {
        try {
            const success = exportImagesToExcel(filteredImages, 'dessert-images', 'الحلويات');
            if (success) {
    ...
    const handleExportCategories = () => {
        try {
            const success = exportCategoriesToExcel(categories, 'dessert-categories');
            if (success) {
    ...
    return (
        <div className="container-fluid p-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <div>
                            <h1 className="display-6 fw-bold text-dark mb-1">
                                <i className="bi bi-cake2 text-warning me-3"></i>
                                إدارة الحلويات
                            </h1>
                            <p className="text-muted fs-5">إدارة قائمة الحلويات والفئات والصور</p>
                        </div>
                        <div className="d-flex gap-2">
                            <div className="btn-group" role="group">
                                <button
                                    ...
                                    disabled={isLoading}
                                    title="تصدير الحلويات إلى Excel"
                                >
                ...
                <div className="col-lg-3 col-md-6">
                    <div className="card border-0 shadow-sm bg-warning text-white">
                        <div className="card-body text-center">
                            <i className="bi bi-cake2 fs-1 mb-2"></i>
                            <h3 className="fw-bold">{products.length}</h3>
                            <p className="mb-0">إجمالي الحلويات</p>
                        </div>
                    </div>
                </div>
                <div className="col-lg-3 col-md-6">
                    ...
                    <div className="card border-0 shadow-sm">
                        <div className="card-header bg-light border-0">
                            <h5 className="card-title mb-0">
                                ...
                                قائمة الحلويات ({filteredProducts.length})
                            </h5>
                        </div>
                            ...
                            <div className="table-responsive">
                                ...
                                {filteredProducts.length === 0 && (
                                    <div className="text-center py-5">
                                        ...
                                        <p className="text-muted mt-3">لا توجد حلويات متطابقة مع البحث</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {/* All Modals with the same pattern as other pages */}
            {showCategoryForm && (
                                ...
                                <div className="modal-body">
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            className="form-control"
                                            placeholder="مثال: كيك، حلويات شرقية، آيس كريم..."
                                            required
                                            disabled={isLoading}
                                        />
                                    </div>
                        ...
                        <div className="modal-content">
                            ...
                            <form onSubmit={handleAddProduct}>
                                <div className="modal-body">
                                    <div className="row g-3">
                                        <div className="col-md-6">
                                            <label className="form-label fw-semibold">اسم الحلوى</label>
                                            <input
                                                type="text"
                                                value={newProduct.name}
                                                onChange={(e) => setNewProduct({ ...newProduct, name: e.target.value })}
                                                className="form-control"
                                                placeholder="مثال: كيك الشوكولاتة، بقلاوة..."
                                                required
                                                disabled={isLoading}
                                            />
                                        </div>
