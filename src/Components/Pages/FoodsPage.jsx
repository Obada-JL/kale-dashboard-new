...
const FoodsPage = () => {
    ...
    const fetchCategories = async () => {
        try {
            const response = await apiService.categories.getFoodCategories();
            setCategories(response.data);
        } catch (error) {
            handleApiError(error, 'جلب فئات الأطعمة');
        }
    };
    ...
    const fetchProducts = async () => {
        try {
            ...
            handleApiError(error, 'جلب الأطعمة');
        }
    };

    const fetchImages = async () => {
        try {
            const response = await apiService.images.food.getAll();
            setImages(response.data);
        } catch (error) {
            handleApiError(error, 'جلب صور الأطعمة');
        }
    };
    ...
    const handleAddProduct = async (e) => {
        ...
        try {
            setIsLoading(true);
            await apiService.foods.add(newProduct);
            setNewProduct({
                name: '',
                nameTr: '',
                price: '',
            ...
            handleApiError(error, 'إضافة طعام');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUpdateProduct = async (e) => {
        ...
        try {
            setIsLoading(true);
            await apiService.foods.update(editingProduct._id, editingProduct);
            setEditingProduct(null);
            await fetchProducts();
            toast.success('تم تحديث الطعام بنجاح');
        } catch (error) {
            handleApiError(error, 'تحديث طعام');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleAddImage = async (e) => {
        ...
        try {
            ...
            await apiService.images.food.add(formData);
            setImageFile(null);
            setSelectedCategory('');
    ...
    const handleDeleteProduct = async (productId, productName) => {
        ...
        try {
            setIsLoading(true);
            await apiService.foods.delete(productId);
            await fetchProducts();
            toast.success('تم حذف الطعام بنجاح');
        } catch (error) {
            handleApiError(error, 'حذف طعام');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeleteImage = async (imageId) => {
        ...
        try {
            setIsLoading(true);
            await apiService.images.food.delete(imageId);
            await fetchImages();
    ...
    const handleExportProducts = () => {
        try {
            const success = exportToExcel(filteredProducts, 'foods', 'الأطعمة');
            if (success) {
                toast.success('تم تصدير الأطعمة بنجاح');
            } else {
                toast.error('فشل في تصدير الأطعمة');
            }
        } catch (error) {
    ...
    const handleExportImages = () => {
        try {
            const success = exportImagesToExcel(filteredImages, 'food-images', 'الأطعمة');
            if (success) {
    ...
    const handleExportCategories = () => {
        try {
            const success = exportCategoriesToExcel(categories, 'food-categories');
            if (success) {
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <div>
                            <h1 className="display-6 fw-bold text-dark mb-1">
                                <i className="bi bi-egg-fried text-primary me-3"></i>
                                إدارة الأطعمة
                            </h1>
                            <p className="text-muted fs-5">إدارة قائمة الأطعمة والفئات والصور</p>
                        </div>
                ...
                <div className="col-lg-3 col-md-6">
                    <div className="card border-0 shadow-sm bg-primary text-white">
                        <div className="card-body text-center">
                            <i className="bi bi-egg-fried fs-1 mb-2"></i>
                            <h3 className="fw-bold">{products.length}</h3>
                            <p className="mb-0">إجمالي الأطعمة</p>
                        </div>
                    </div>
                </div>
                <div className="col-lg-3 col-md-6">
                    ...
                    <div className="card border-0 shadow-sm">
                        <div className="card-header bg-light border-0">
                            <h5 className="card-title mb-0">
                                ...
                                قائمة الأطعمة ({filteredProducts.length})
                            </h5>
                        </div>
                            ...
                            <div className="table-responsive">
                                ...
                                {filteredProducts.length === 0 && (
                                    <div className="text-center py-5">
                                        ...
                                        <p className="text-muted mt-3">لا توجد أطعمة متطابقة مع البحث</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="modal-content">
                            ...
                            <form onSubmit={handleAddProduct}>
                                <div className="modal-body">
                                    <div className="row g-3">
                                        <div className="col-md-6">
                                            <label className="form-label fw-semibold">اسم الطعام</label>
                                            <input
                                                type="text"
                                                value={newProduct.name}
                                                onChange={(e) => setNewProduct({ ...newProduct, name: e.target.value })}
                                                className="form-control"
                                                placeholder="مثال: كبسة لحم، برجر دجاج..."
                                                required
                                                disabled={isLoading}
                                            />
                                        </div>
                                        <div className="col-md-6">
                                            ...
                                            <input
                                                ...
                                                className="form-control"
                                                placeholder="Örn: Kuzu kebap, Tavuk burger..."
                                                disabled={isLoading}
                                            />
                                        </div>
                                        <div className="col-md-6">
